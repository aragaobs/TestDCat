...
function treeBoxes(urlService, jsonData)
{
	...
	var rectNode = { width : 130, height : 10, textMargin : 5 };
...
/*		nodeGroupTooltip = svgGroup.append('g')
	...
	function calculo(d){
		return (d.code.length/15)*10 + (d.nodeName.length/25)*15+20
	}
...
/*		var nodeEnterTooltip = nodesTooltip.enter().append('g')
	...
	*/
		...
		//eh aqui que muda o tamanho do retangulo
		...
		.attr('height', function(d) {
					return (rectNode.textMargin * 2) < 0 ? 0
							: calculo(d)
				})
		...
		.style('fill', 'white')
		.append("tspan")
	    .text(function(d) {return 'Description: ' + d.name;})
	    .append("tspan")
	    .attr('x', rectNode.width / 2 + tooltip.textMargin)
	    .attr('dy', '1.5em');*/
...
/*		linkTooltip.exit().transition()
	...
	function collision(siblings) {
	  var minPadding = 5;
	 //alert(siblings[0].toSource())
	  if (siblings) {
		  for (var i = 0; i < siblings.length - 1; i++)
		  {
			  if (siblings[i + 1].x - (siblings[i].x + calculo(siblings[i])) < minPadding)
				  siblings[i + 1].x = siblings[i].x + calculo(siblings[i]) + minPadding;
		  }
	  }
	}
	...
	function diagonal(d) {
		//alert(d.toSource())
		var p0 = {
			x : d.source.x + calculo(d.source) / 2,
			y : (d.source.y + rectNode.width)
		}, p3 = {
			x : d.target.x + calculo(d.source)  / 2,
